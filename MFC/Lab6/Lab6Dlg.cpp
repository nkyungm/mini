...
BEGIN_MESSAGE_MAP(CLab6Dlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_EN_CHANGE(IDC_EDIT1, &CLab6Dlg::OnEnChangeEdit1)
	ON_BN_CLICKED(IDC_START, &CLab6Dlg::OnBnClickedStart)
END_MESSAGE_MAP()
...
void CLab6Dlg::OnBnClickedStart()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	if (UpdateData(TRUE) == 0) //DDX/DDV를 직접 동작
		return; //오류 발생시 리턴
	if (m_num1 >= m_num2) {
		MessageBox(_T("숫자1이 숫자2보다 작아야합니다"), _T("오류"), MB_ICONERROR);
		return;
	}
	...
	for (int i = 0; i < m_num3; i++) {
		...
		m_list.AddString(str); // 생성된 난수를 리스트 박스에 출력
	}
}
